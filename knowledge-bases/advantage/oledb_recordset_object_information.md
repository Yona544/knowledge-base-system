...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| Recordset Object Information  Advantage OLE DB Provider (for ADO) |  |  | Feedback on: Advantage Database Server 12 - Recordset Object Information Advantage OLE DB Provider (for ADO) oledb\_Recordset\_object\_information Advantage OLE DB Provider (for ADO) > Using the Advantage OLE DB Provider with ADO > Recordset Object Information / Dear Support Staff, |  |
...
| GetRows | Yes |  |
| GetString | Yes |  |
| Move | Yes | When using server-side cursors, a Move method will move the current Recordset pointer via an Advantage [Skip](master_skip_movement.htm) operation. If the number of records to Move is a large value (e.g., several hundreds or thousands of records) in a large table, this operation may be slow. To move to a specific record in a Recordset, use the Recordset.Bookmark property. |
| MoveFirst | Yes | When using server-side cursors, a MoveFirst method will move the current Recordset pointer to the first record in the recordset via an Advantage [Go Top](master_go_top_movement.htm) operation. This operation should be very fast and efficient. |
| MoveLast | Yes | When using server-side cursors, a MoveLast method will move the current Recordset pointer to the last record in the recordset via an Advantage [Go Bottom](master_go_bottom_movement.htm) operation. Unless this method causes an SQL static cursor to be dynamically populated, this operation should be very fast and efficient. |
| MoveNext | Yes | When using server-side cursors, a MoveNext method will move the current Recordset pointer to the next record in the recordset via an Advantage [Skip](master_skip_movement.htm) operation. This operation should be very fast and efficient. |
| MovePrevious | Yes | When using server-side cursors, a MovePrevious method will move the current Recordset pointer to the previous record in the recordset via an Advantage [Skip](master_skip_movement.htm) operation. This operation should be very fast and efficient. |
